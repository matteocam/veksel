...
def window(x, y):
    ...
    ln.append('pub const FP_INNER_BITS: usize = %d;' % int(p).bit_length())
    ln.append('')
    ln.append('// scalar field of the inner curve')
    ln.append('pub use fp_inner::Fp256 as Fp;')
    ln.append('')
    ln.append('// p = %d' % l)
    ln.append('fp31!(')
    ...
    ln.append('}')
    ln.append('')
    ln.append('pub fn generator() -> PointValue {')
    ln.append('  PointValue {')
    ln.append('    x: Scalar::from_bits([%s]),' % hex_bytes(int(x0)))
    ln.append('    y: Scalar::from_bits([%s])' % hex_bytes(int(y0)))
    ln.append('  }')
    ln.append('}')
